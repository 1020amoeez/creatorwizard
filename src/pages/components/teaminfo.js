...
const Teaminfo = ({ onNext, formDataname, setFormDataName, id, draftdata }) => {
  ...
  const handleSaveAndProceed = async () => {
    for (let i = 0; i < teamMembers?.length; i++) {
      const { name, designation, imageUrl } = teamMembers[i];
      if (
    ...
    for (let i = 0; i < teamMembers?.length; i++) {
      ...
      const updatedMember = { name, designation };
      updatedMember.imageUrl = imageUrl;
      if (twitterUrl) {
        updatedMember.twitterUrl = twitterUrl;
      }
              ...
              <div className="team-member" key={index}>
                ...
                <div className="stepinputmain">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleInputChange(index, "twitterUrl", e.target.value)
                    }
                  />
                </div>
              </div>
